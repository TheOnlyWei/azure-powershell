if(($null -eq $TestName) -or ($TestName -contains 'Get-AzAksArcClusterUserKubeconfig'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Get-AzAksArcClusterUserKubeconfig.Recording.json'
...
Describe 'Get-AzAksArcClusterUserKubeconfig' {
    It 'Get' {
        ...
        $config = Get-AzAksArcClusterUserKubeconfig `
